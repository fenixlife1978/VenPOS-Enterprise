...
export default function DashboardAdmin({ 
  ...
  return (
    <div className="flex h-screen bg-[#d4d4d4]">
      {/* Sidebar Azul Colapsable - SIN CAMBIOS */}
      <aside 
        className={`${
...
// Dashboard Home - Estilo exacto MasterPOS referencia imagen 2
function DashboardHome({ store, formatMoney, updateStore }: { store: AppStore; formatMoney: (amount: number) => string; updateStore: (updater: any) => void }) {
  ...
  const handleUpdateRate = () => {
    ...
    if (!isNaN(newRate) && newRate > 0) {
      updateStore((prev: any) => ({
        ...prev,
        config: { ...prev.config, exchangeRate: newRate }
      }));
  ...
  return (
    <div className="space-y-6">
      {/* Header Panel de Administración - Estilo exacto imagen 2 */}
      <div className="bg-[#d4d4d4] rounded-2xl p-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {/* Botón Historial Cierres - AZUL sólido */}
          ...
          {/* Botón Reset Sistema - ROJO sólido */}
      ...
      {/* Stats Grid - Layout 2 columnas como imagen 2 */}
            ...
            <tbody className="divide-y divide-gray-100">
              {store.sales.slice(-5).reverse().map((sale) => (
                <tr key={sale.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-mono text-xs text-gray-900">#{sale.id}</td>
                  ...
                  <td className="px-4 py-3 text-right font-bold text-xs text-gray-900">{formatMoney(sale.total)}</td>
            ...
            <tbody className="divide-y divide-gray-100">
              {store.sales.slice().reverse().map(s => (
                <tr key={s.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-mono text-xs text-[#0a1628]">#{s.id}</td>
                  <td className="px-4 py-3 text-gray-600">{new Date(s.date).toLocaleDateString()}</td>
                  <td className="px-4 py-3 text-gray-600">{s.cashier}</td>
                  <td className="px-4 py-3 font-medium text-gray-800">{s.customerName}</td>
                  <td className="px-4 py-3 text-right font-bold text-[#0a1628]">{formatMoney(s.total)}</td>
                  <td className="px-4 py-3 text-center">
                    <Badge className="bg-green-100 text-green-700 hover:bg-green-100">Completada</Badge>
                  </td>
                </tr>
...
function ReportsModule({ store, formatMoney }: { store: AppStore; formatMoney: (amount: number) => string }) {
  ...
  const todaySales = store.sales.filter(s => new Date(s.date).toDateString() === new Date().toDateString()).reduce((sum, s) => sum + s.total, 0);
...
// Users Module - SIN CAMBIOS
function UsersModule({ store }: { store: AppStore }) {
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-2xl font-black text-[#0a1628]">Usuarios</h2>
        <Button className="bg-[#0a1628] hover:bg-[#1e3a5f] rounded-xl">
            ...
            <tbody className="divide-y divide-gray-100">
              {store.users.map(u => (
                <tr key={u.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3 font-medium text-[#0a1628]">{u.username}</td>
                  ...
                  <td className="px-4 py-3">
                    <Badge className={u.role === 'admin' ? 'bg-blue-100 text-blue-700 hover:bg-blue-100' : 'bg-gray-100 text-gray-700 hover:bg-gray-100'}>
                      {u.role === 'admin' ? 'Administrador' : u.role === 'cashier' ? 'Cajero' : 'Vendedor'}
                    </Badge>
                  </td>
                  <td className="px-4 py-3">
